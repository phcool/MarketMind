...
def build_section_summary_messages(
    ...
    if section == "comments":
        system_prompt = (
            ...
            "围绕股票未来走势总结市场讨论中的利好/利多因素、利空因素、情绪方向和分歧点。"
            "不要编造未提供的信息，使用简洁中文输出。"
        )
        lines = [
            ...
            f"评论数量：{len(items)}",
            "",
            "请基于下面这些评论标题、发布时间、点击量和评论量，围绕“利好/利多”和“利空”总结市场讨论。",
            "输出要求：",
            "1. 先给出“整体多空倾向”，判断评论情绪偏利好、偏利空还是分歧明显；",
            "2. 列出“利好/利多因素”，只基于评论中能看出的正面预期或做多理由；",
            "3. 列出“利空因素”，只基于评论中能看出的负面预期或风险担忧；",
            "4. 补充“分歧与不确定性”，说明哪些观点缺少事实支撑或存在明显争议；",
            "5. 最后给出“对未来趋势判断的启示”，简述这些评论对后续走势的可能影响。",
            "",
            "评论列表：",
        ]
    ...
    if section == "news":
        system_prompt = (
            ...
            "围绕股票未来走势提炼事件主线、利好/利多因素、利空因素和不确定性。"
            "不要编造未提供的信息，使用简洁中文输出。"
        )
        lines = [
            f"股票：{company_name}（{stock}）",
            f"新闻数量：{len(items)}",
            "",
            "请基于下面这些新闻，围绕“利好/利多”和“利空”总结它们对股价方向的潜在影响。",
            "输出要求：",
            "1. 先给出“核心事件主线”；",
            "2. 列出“利好/利多因素”，说明哪些新闻可能支持上涨或改善预期；",
            "3. 列出“利空因素”，说明哪些新闻可能压制股价或增加风险；",
            "4. 补充“分歧与不确定性”，区分事实、市场解读和仍待验证的信息；",
            "5. 最后给出“对未来趋势判断的启示”，简述这些新闻对后续走势的可能影响。",
            "",
            "新闻列表：",
        ]
        for idx, item in enumerate(items, start=1):
    ...
    if section == "reports":
        system_prompt = (
            ...
            "围绕股票未来走势提炼机构观点中的利好/利多逻辑、利空风险和关键假设。"
            "不要编造未提供的信息，使用简洁中文输出。"
        )
        lines = [
            f"股票：{company_name}（{stock}）",
            f"研报数量：{len(items)}",
            "",
            "请基于下面这些研报，围绕“利好/利多”和“利空”总结机构观点对股价方向的潜在影响。",
            "输出要求：",
            "1. 先给出“机构总体判断”，说明观点偏积极、偏谨慎还是分歧明显；",
            "2. 列出“利好/利多因素”，包括业绩、估值、行业景气、订单、政策等正面逻辑；",
            "3. 列出“利空因素”，包括业绩压力、估值风险、竞争、需求、政策或执行风险；",
            "4. 补充“关键假设与不确定性”，说明哪些判断依赖未来兑现；",
            "5. 最后给出“对未来趋势判断的启示”，简述这些研报观点对后续走势的可能影响。",
            "",
            "研报列表：",
        ]
        for idx, item in enumerate(items, start=1):
    ...
    if section == "serp":
        system_prompt = (
            ...
            "围绕股票未来走势提炼外部网页中的事件线索、利好/利多因素、利空因素和证据可靠性。"
            "不要编造未提供的信息，使用简洁中文输出。"
        )
        lines = [
            f"股票：{company_name}（{stock}）",
            f"搜索结果数量：{len(items)}",
            "",
            "请基于下面这些网页正文，围绕“利好/利多”和“利空”总结外部搜索结果对股价方向的潜在影响。",
            "输出要求：",
            "1. 先给出“外部信息主线”；",
            "2. 列出“利好/利多因素”，说明哪些网页信息可能支持上涨或改善预期；",
            "3. 列出“利空因素”，说明哪些网页信息可能压制股价或增加风险；",
            "4. 补充“证据可靠性与不确定性”，提醒来源质量、重复信息、时效性或弱相关内容；",
            "5. 最后给出“对未来趋势判断的启示”，简述这些外部搜索结果对后续走势的可能影响。",
            "",
            "网页列表：",
        ]
        for idx, item in enumerate(items, start=1):
...
def build_prediction_messages(
    ...
    *,
    ...
    system_prompt = (
        ...
        "对股票未来一段时间的整体趋势做综合预测。"
        "只能使用已提供的信息，不要编造事实，不要给出投资建议，不要给出具体买卖指令。"
        "请使用简洁中文输出。"
    )
    lines = [
        f"股票：{company_name}（{stock}）",
        ...
        "围绕利好/利多因素、利空因素、信息分歧和关键不确定性进行综合判断。",
        "",
        "请严格按下面结构输出：",
        "1. 趋势预测：用一句话判断未来走势倾向，可使用“偏强/偏弱/震荡/分歧较大”等表述；",
        "2. 主要利好/利多依据：列出 2-4 条；",
        "3. 主要利空或风险：列出 2-4 条；",
        "4. 关键不确定性：说明哪些信息仍需观察或验证；",
        "5. 综合结论：给出简短、谨慎的趋势判断，不要输出具体交易建议。",
        "",
        "如果部分 Summary 缺失，请基于剩余 Summary 与 K 线继续判断；不要因为信息缺失拒绝回答。",
        "",
    ]
    return system_prompt, "\n".join(lines)
