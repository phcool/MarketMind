...
DEFAULT_DATASET = ROOT_DIR / "dataset" / "quotes_summary_5d_2026-03-01_to_2026-04-01.csv"
DEFAULT_OUTPUT_JSON = ROOT_DIR / "test" / "outputs" / "quotes_summary_5d_202603_vllm_eval.json"
DEFAULT_MODEL = "/nfs/hanpeng/huggingface/models/qwen2_5_grpo_quotes_summary_multi_weighted_reward"

TRIPLE_LABEL_RE = re.compile(r"^\s*([涨跌])\s*[，,/\s]\s*([涨跌])\s*[，,/\s]\s*([涨跌])\s*$")
EXTRACTION_FAILED = "提取失败"
HORIZON_WEIGHTS = (0.50, 0.30, 0.20)
...
def _compute_scores(results: list[dict[str, object]], *, include_failed: bool) -> dict[str, object]:
    total_samples = 0
    total_unweighted = 0.0
    total_weighted = 0.0
    horizon_correct = [0, 0, 0]
    horizon_total = [0, 0, 0]
    ...
    for row in results:
        ...
        if pred_parts is None:
            continue
        matches = [a == b for a, b in zip(pred_parts, ref_parts)]
        total_unweighted += sum(1 for ok in matches if ok) / 3.0
        total_weighted += sum(weight for ok, weight in zip(matches, HORIZON_WEIGHTS) if ok)
        for idx, ok in enumerate(matches):
            horizon_total[idx] += 1
            if ok:
                horizon_correct[idx] += 1

    horizon_accuracy = [
        (horizon_correct[idx] / horizon_total[idx]) if horizon_total[idx] else 0.0
        for idx in range(3)
    ]
    return {
        "unweighted_correct": total_unweighted,
        "weighted_correct": total_weighted,
        "total": total_samples,
        "unweighted_accuracy": (total_unweighted / total_samples) if total_samples else 0.0,
        "weighted_accuracy": (total_weighted / total_samples) if total_samples else 0.0,
        "horizon_accuracy": {
            "future_1d": horizon_accuracy[0],
            "future_3d": horizon_accuracy[1],
            "future_7d": horizon_accuracy[2],
        },
        "horizon_correct": {
            "future_1d": horizon_correct[0],
            "future_3d": horizon_correct[1],
            "future_7d": horizon_correct[2],
        },
        "horizon_total": {
            "future_1d": horizon_total[0],
            "future_3d": horizon_total[1],
            "future_7d": horizon_total[2],
        },
    }


def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate a vLLM model on the 2026-03 quotes+summary dataset split.")
...
def main() -> None:
    ...
    model_path = str(Path(args.model).expanduser().resolve()) if Path(args.model).expanduser().exists() else args.model
    ...
    print(f"[eval] Generating with {model_path} on {len(prompts)} prompts ...")
    ...
    scores_all = _compute_scores(results, include_failed=True)
    scores_valid = _compute_scores(results, include_failed=False)
    ...
    payload = {
        ...
        "reward_weights": {
            "future_1d": HORIZON_WEIGHTS[0],
            "future_3d": HORIZON_WEIGHTS[1],
            "future_7d": HORIZON_WEIGHTS[2],
        },
        "scores_including_extraction_failures": scores_all,
        "scores_excluding_extraction_failures": scores_valid,
        "results": results,
    }
    ...
    print(
        "Weighted score (including extraction failures): "
        f"{scores_all['weighted_accuracy']:.2%} ({scores_all['weighted_correct']:.4f}/{scores_all['total']})"
    )
    print(
        "Unweighted score (including extraction failures): "
        f"{scores_all['unweighted_accuracy']:.2%} ({scores_all['unweighted_correct']:.4f}/{scores_all['total']})"
    )
