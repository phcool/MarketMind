...
DEFAULT_OUTPUT_JSON = ROOT_DIR / "test" / "outputs" / "quotes_summary_5d_vllm_no_text_eval.json"
...
TAIL = """请综合K线形态、量能变化与短期走势，分别预测该股票相对当前交易日收盘价在以下三个阶段的涨跌：
1. 1个交易日后
2. 3个交易日后
3. 7个交易日后
...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate Qwen2.5-7B-Instruct with vLLM on quotes-only dataset view.")
...
def main() -> None:
    ...
    payload = {
        ...
        "mode": "quotes_only_no_news_no_reports",
