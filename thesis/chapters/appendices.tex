...
该文件由 \path|scripts/batch/batch_qwen_cot_quotes_dataset.py| 生成。脚本将基础 7 日 K 线数据集中的 \texttt{prompt,label} 封装为 DashScope Batch 请求，使用 \texttt{qwen3-max} 生成解释性推理；批处理完成后，再把远端返回结果与原始样本合并为 SFT 所需的 \texttt{prompt,completion} 格式。
