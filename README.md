...
**股票清单**：`config/stocks.json`（`sectors` → 每板块内 `name` / `symbol` / `market`）。抓取脚本与 `src/app.py` 实时页面均从此文件读取，不再扫描本地板块目录。
...
- 行情/股吧/新闻/研报数据在 **`exports/quotes.csv`、`exports/comments.csv`、`exports/news.csv`、`exports/report.csv`**；实时页面会按需抓取最新 comments、news 和 reports，并复用正文缓存。
...
- 输入股票代码，或点击股票池生成的快捷按钮
- 实时抓取最近 `comments / news / reports`
- 自动抓取并缓存新闻正文、研报正文
- 以卡片形式展示标题、链接、日期、互动量和正文内容
